...
// This app calls the proxy — locally it's localhost:3001, on Cloud Run it's the same origin
const PROXY_URL = import.meta.env.VITE_PROXY_URL || "http://localhost:3001/chat";
...
const TOPIC_MODES = {
  ai: {
    ...
    writer: `You write daily LinkedIn posts about AI trends. Your style is plain and human. You sound like a curious professional sharing what they read this week, not a content marketer or an AI. Write in British English throughout (organise, colour, whilst, recognise, behaviour, centre, etc). Rules: never use em dashes, never use words like "groundbreaking", "game-changer", "revolutionising", "dive into", "landscape", "delve", "transformative", or "cutting-edge". Start with exactly this line on its own: "5 AI topics and trends worth knowing about this week:" Then number each trend 1. through 5. — start each item with a relevant emoji that fits the topic, then bold the key product name or concept using **bold**, then bold any other important keywords or technical terms in the explanation using **bold** too. Give a plain one to two sentence explanation per item. End with a genuine one-liner question to start a conversation. Add 3 to 4 hashtags on their own line at the end.`,
    userPrompt: `Today is ${new Date().toDateString()}. Generate 5 fresh, varied AI trending topics as a JSON array. Be specific with product names and real-world implications. Do not use em dashes.`,
  },
  business: {
    label: "📈 Business & Leadership",
    researcher: `You are a researcher tracking the most talked-about business and leadership trends professionals care about this week. Surface 5 stories covering strategy, workplace culture, leadership thinking, and business innovation. Write in British English. Return ONLY a valid JSON array of 5 objects with keys "title" (one sharp sentence, no em dashes) and "blurb" (one to two plain sentences on why it matters, no hype words, no em dashes). No markdown, no extra text outside the JSON array.`,
    writer: `You write daily LinkedIn posts about business and leadership trends. Your style is plain and human. Write in British English throughout. Rules: never use em dashes, no buzzwords. Start with exactly: "5 business and leadership topics worth knowing about this week:" Number each trend 1. through 5. — start each with a relevant emoji, bold the key concept using **bold**, bold other important terms too, then give a plain one to two sentence explanation. End with a genuine question to spark discussion. Add 3 to 4 hashtags at the end.`,
    userPrompt: `Today is ${new Date().toDateString()}. Generate 5 fresh business and leadership trending topics as a JSON array. Be specific and relevant to professionals. No em dashes.`,
  },
  custom: {
    label: "✏️ Custom Topic",
    researcher: `You are a researcher surfacing the 5 most interesting and timely stories on the topic provided by the user. Write in British English. Return ONLY a valid JSON array of 5 objects with keys "title" (one sharp sentence, no em dashes) and "blurb" (one to two plain sentences on why it matters, no hype words, no em dashes). No markdown, no extra text outside the JSON array.`,
    writer: `You write daily LinkedIn posts. Your style is plain and human. Write in British English throughout. Rules: never use em dashes, no buzzwords. Start the post with "5 topics and trends worth knowing about this week:" Number each trend 1. through 5. — start each with a relevant emoji, bold the key concept using **bold**, bold other important terms too, then give a plain one to two sentence explanation. End with a genuine question. Add 3 to 4 hashtags at the end.`,
    userPrompt: `Today is ${new Date().toDateString()}. Generate 5 fresh trending topics as a JSON array based on the theme given. No em dashes.`,
  },
};
...
export default function App() {
  ...
  const runAgent = async () => {
    ...
    try {
      ...
      const userPrompt = topicMode === "custom" && customTopic
        ? `Today is ${new Date().toDateString()}. Generate 5 fresh trending topics about "${customTopic}" as a JSON array. No em dashes.`
        : mode.userPrompt;

      // ── RESEARCHER ───────────────────────────────────────────────
      activate("researcher");
      addLog(`🔍 Researcher starting — topic: ${mode.label}...`, "agent");
      addLog("📡 Fetching latest trend signals...", "info");

      const topicsRaw = await callAzure(mode.researcher, userPrompt);
        ...
        {tab === "agent" && (
          <div>
            ...
            <div style={{ marginBottom: 16 }}>
              <div style={{ fontSize: 11, color: "#475569", fontFamily: "monospace", marginBottom: 8, letterSpacing: 1 }}>TOPIC MODE</div>
              ...
              {topicMode === "custom" && (
                <input
                  type="text"
                  placeholder="e.g. cybersecurity, fintech, climate tech..."
                  value={customTopic}
